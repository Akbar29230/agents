...
async function callClaude(systemPrompt, userText) {
  ...
  if (data.error) {
    ...
    return "Сервер вернул ошибку. Проверь, что ANTHROPIC_API_KEY задан в переменных окружения Vercel.";
  }
  return data.text || "Не удалось получить ответ. Попробуй ещё раз.";
}
